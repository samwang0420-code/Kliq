...
export function CropControl({
	...
	useEffect(() => {
		...
		const currentMs = typeof currentTimeMs === "number" ? currentTimeMs : 0;
	...
	const handlePointerMove = (e: React.PointerEvent) => {
		...
		if (followEnabled) {
			...
			switch (isDragging) {
				case "top": {
					newCrop.height = Math.max(0.1, Math.min(1, initialCrop.height - deltaY));
					break;
				}
				case "bottom": {
					newCrop.height = Math.max(0.1, Math.min(1, initialCrop.height + deltaY));
				...
				case "left": {
					newCrop.width = Math.max(0.1, Math.min(1, initialCrop.width - deltaX));
					break;
				}
				case "right": {
					newCrop.width = Math.max(0.1, Math.min(1, initialCrop.width + deltaX));
				...
				<div className="mx-auto mb-4 flex max-w-3xl flex-col gap-3 rounded-lg border border-border bg-card/40 p-4">
					...
					<label className="flex items-center justify-between gap-3 text-sm">
						<span className="flex flex-col">
							<span className="font-medium">
								{t("settings.crop.cursorFollow.textZoom")}
							</span>
							<span className="text-xs text-muted-foreground/60">
								{t("settings.crop.cursorFollow.textZoomHint")}
							</span>
						</span>
						<input
							type="checkbox"
						...
						<>
							<div className="flex items-center gap-3 text-xs">
								<span className="w-28 text-muted-foreground">
									{t("settings.crop.cursorFollow.outputSize")}
								</span>
							...
							<div className="flex items-center gap-3 text-xs">
								<span className="w-28 text-muted-foreground">
									{t("settings.crop.cursorFollow.safeZone")}
								</span>
								<input
									type="range"
									min={0}
									...
									onChange={(e) => handleSafeZoneChange(Number(e.target.value))}
									className="flex-1"
								/>
							...
							<div className="flex items-center gap-3 text-xs">
								<span className="w-28 text-muted-foreground">
									{t("settings.crop.cursorFollow.smoothness")}
								</span>
								<input
									type="range"
									min={0}
							...
							<label className="flex items-center gap-3 text-xs">
								<span className="w-28 text-muted-foreground">
									{t("settings.crop.cursorFollow.textCursorFocus")}
								</span>
								<input
									type="checkbox"
								...
								<span className="text-muted-foreground/60">
									{t("settings.crop.cursorFollow.textCursorFocusHint")}
								</span>
							</label>
							<div className="flex items-center gap-2 text-xs">
								<span className="w-28 text-muted-foreground">
									{t("settings.crop.cursorFollow.preview")}
								</span>
								<div className="inline-flex overflow-hidden rounded-md border border-border">
									...
									>
										{t("settings.crop.cursorFollow.previewSource")}
									</button>
									<button
										type="button"
									...
									>
										{t("settings.crop.cursorFollow.previewOutput")}
									</button>
								</div>
							</div>
						</>
